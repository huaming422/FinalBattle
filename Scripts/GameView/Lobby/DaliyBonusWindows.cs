    ...
    public class DaliyBonusWindow : BaseWindow
    {
            ...
            foreach (string itemStr in bonus.n32ItemID)
            {
                ...
                if (itemID == 1)
                {
                    EventCenter.Broadcast<EPurchaseType, string, string, int>(EGameEvent.eGameEvent_PurchaseRuneSuccessWin, 
                        EPurchaseType.EPT_Gold, "동전", "", count);
                }
                else if (itemID == 2)
                {
                    EventCenter.Broadcast<EPurchaseType, string, string, int>(EGameEvent.eGameEvent_PurchaseRuneSuccessWin, 
                        EPurchaseType.EPT_Crystal, "보석", "", count);
                }
                ...
                {
                    EventCenter.Broadcast<EPurchaseType, string, string, int>(EGameEvent.eGameEvent_PurchaseRuneSuccessWin, 
                        EPurchaseType.EPT_Rune, "장비", "", count);
                }
        ...
        private void OnEvent_DailyBonusUpdate()
        {
            ...
            for (uint i = 0; i < DailyBonusCtrl.Instance.mMaxDay; ++i)
            {
                ...
                //addobj.transform.Find("BG/Label").GetComponent<UILabel>().text = "일" + (i + 1).ToString() + "가입선물";
                addobj.transform.Find("BG/Label").GetComponent<UILabel>().text = (i + 1).ToString() + "일" + "가입선물";
                ...
                foreach (string itemStr in bonus.n32ItemID)
                {
                    ...
                    if (itemID == 1)
                    {
                        ...
                        oneBouns.transform.Find("Name").GetComponent<UILabel>().text = "동전";
                    }
                    else if (itemID == 2)
                    {
                        oneBouns.transform.GetComponent<UISprite>().spriteName = (10).ToString();
                        oneBouns.transform.Find("Name").GetComponent<UILabel>().text = "보석";
                    }
                    else if (itemID > 100000 && itemID < 110000)
                    {
                    ...
                    {
                        oneBouns.transform.Find("Name").GetComponent<UILabel>().text = "복장";
                        oneBouns.transform.GetComponent<UISprite>().spriteName = ConfigReader.HeroSkinXmlInfoDict[(int)itemID].Icon;
                    }
                    else if (itemID > 120000 && itemID < 130000)
                    {
                        oneBouns.transform.Find("Name").GetComponent<UILabel>().text = "장비";
