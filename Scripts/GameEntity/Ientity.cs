...
namespace BlGame.GameEntity
{
    ...
    public class Ientity
    {
        ...
        public AudioSource absorbSound;//흡수할때나는소리
        ...
        public float EntityOverplusRemainTime//entityoverplus여분의 과잉의
        {
            set;
            get;
        }
        ...
        public float StrategyTick//국가전략,전략
        {
            set;
            get;
        }
        ...
        //변화원인(0:보퉁상하다;1:BUFF잃어버린피;2:날카로운공격;3:회복)
        ...
        /// 角色中了束缚Buff
        /// </summary>
        public void OnEntityGetAstrictBuff()//astrict좁은곳에 가두어 놓다.속박시키다.
        {
        ...
        protected int skillState;//기능상태( 1:开始吟唱 2:开始施法(前摇) 3:释放技能)
        ...
        public void SetJungleMonsterBeAtkVoice()
        {
            ...
            int indexPlay = GameMethod.RandProbablityIndex(ifPlaySoundList);//소리를 내보낼 첨수를 얻는다.
            if (indexPlay == 1)
            {
                return;
            }
            ...
            List<float> probabilityList = GameMethod.ResolveToFloatList(info.n32Script1Rate);//집합확률을 얻는다.
            for (int i = 0; i < probabilityList.Count; i++)
            {
                probabilityList[i] = (float)(probabilityList[i] - 90000) / 100f;
            }
            int index = GameMethod.RandProbablityIndex(probabilityList);//첨수를 얻는다.
            List<string> voiceList = GameMethod.ResolveToStrList(info.un32Script1);
            string name = voiceList[index];//목소리에대응되는첨수를 얻는다.
