...
namespace BlGame.Network
{
    public class NetworkManager : Singleton<NetworkManager>
    {
        ...
        //private string m_IP = "10.30.16.181";
        ...
        public NetworkManager()
        {
            ...
            //预先创建消息RuntimeTypeModel运行시类型模型    
            //A
            ProtoBuf.Serializer.PrepareSerializer<GSToGC.AbsorbBegin>();//absorb->흡수하다,빨아들이다
        ...
        public void Connect()
        {
            ...
            Debugger.Log("IClientSession Connect");//Session=> 망주소를 가지는 두 NAU(망주소지정가능단위)사이의 일시적인 론리적접속관계를 말함.
...
//#if LOG_FILE && UNITY_EDITOR
                if (n32MsgID != 8192 && n32MsgID != 16385)
                {
                    ...
                    {
                        sw.WriteLine(Time.time + "  전송：\t" + n32MsgID + "\t" + msgName);
                    }
                }
//#endif
...
//#if LOG_FILE && UNITY_EDITOR
                if (type != 1)
                {
                    ...
                    using (System.IO.StreamWriter sw = new System.IO.StreamWriter(@"E:\Log.txt", true))
                    {
                        sw.WriteLine(Time.time + "  수신：\t" + type + "\t" + msgName);
                    }
                }
//#endif
